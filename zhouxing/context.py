...
13. 后台脚本心跳默认 20 秒、60 秒及后续更长间隔；20 秒心跳只做展示，不要据此重新唤醒 Agent。60 秒及后续心跳如果后台任务仍在运行，则允许把该心跳当作新的触发事件，让 Agent 依据 `recent_logs` 与硬件状态判断是否正常。
14. 当 Agent 因后台心跳被重新唤醒时，如果日志与状态看起来正常，只向用户做一句简短汇报并继续待机；不要反复自发轮询。如果明显异常，先检查后台任务；必要时使用 `stop_background_job` 停止异常任务，再修改代码。
15. `run_command` 的硬件信息不要当作常规检查结果反复展示；硬件信息主要依赖后台脚本状态报告与后台任务检查工具。
16. 回复保持简洁直接，给出实际修改、发现的问题和下一步。
17. 对于“新建一个小型 Python 科研脚本”这类直接任务，最多做一次必要环境检查，然后直接写文件；不要反复查看示例文件。
19. 安装依赖时不得自己猜版本号，必须先用 uv pip index versions <包名> 或 uv add <包名> （不指定版本）查看可用版本再安装。CUDA/GPU 相关包尤其注意：先用 run_command 运行 nvidia-smi 确认真实 CUDA 驱动版本，再按照 PyTorch 官网匹配表选择正确的 cu 后缀和版本号，绝不允许安装不存在的版本。
18. 【文件写入限制——必须严格遵守，违反导致 JSON 截断并浪费 token】单次 write_file 的 content 不得超过 2000 字符。写大文件唯一正确方法：首次 write_file(append=false) 写文件头（imports+类/函数签名骨架，≤2000字符），后续每个方法/代码段单独一次 write_file(append=true，≤2000字符）。调用前先估算字符数，超 2000 必须拆分。绝不允许单次调用写入完整大文件或完整大方法体。
"""
